  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr] max-h-[85vh]">
              ...
              {models.map((m) => (
                <button
                  ...
                  className={cn(
                    ...
                    m.id === sel ? "bg-primary-container/50 ring-1 ring-primary/20" : "hover:bg-surface-container"
                  )}
                >
          ...
          <div className="p-6 flex flex-col overflow-y-auto">
            ...
            <p className="font-body text-sm text-on-surface-variant leading-relaxed mb-4">{selected.description}</p>
              ...
              {mode === "image" && (
                <>
                  <Row label={t.aigc.aspectRatio}>
                    <MiniSelect value={draft.aspect ?? "16:9"} options={ASPECT_OPTIONS} onChange={(v) => set({ aspect: v })} />
                  </Row>
                  <Row label={t.aigc.quality}>
                    <MiniSelect value={draft.quality ?? "2K"} options={QUALITY_OPTIONS} onChange={(v) => set({ quality: v })} />
                  </Row>
                  <Row label={t.aigc.numberOfImages}>
                    <MiniSelect value={draft.count ?? 1} options={COUNT_OPTIONS} onChange={(v) => set({ count: v })} />
                  </Row>
                </>
              )}
              {mode === "video" && (
                <>
                  ...
                  <Row label={t.aigc.duration}>
                    <MiniSelect value={draft.duration ?? "5 sec"} options={VIDEO_DURATION_OPTIONS} onChange={(v) => set({ duration: v })} />
                  </Row>
                  <Row label={t.aigc.resolution}>
                    <MiniSelect value={draft.resolution ?? "1080p"} options={VIDEO_RESOLUTION_OPTIONS} onChange={(v) => set({ resolution: v })} />
                  </Row>
                </>
              )}
              {mode === "voiceover" && (
                <>
                  <Row label={t.aigc.language}>
                    <MiniSelect value={draft.language ?? "English"} options={LANGUAGE_OPTIONS} onChange={(v) => set({ language: v })} />
                  </Row>
                  <Row label={t.aigc.accent}>
                    <MiniSelect value={draft.accent ?? "American"} options={ACCENT_OPTIONS} onChange={(v) => set({ accent: v })} />
                  </Row>
                  <Row label={t.aigc.effect}>
                    <MiniSelect value={draft.effect ?? "No Effect"} options={VOICE_EFFECT_OPTIONS} onChange={(v) => set({ effect: v })} />
                  </Row>
                </>
              )}
              {mode === "music" && (
                <>
                  <Row label={t.aigc.genre}>
                    <MiniSelect value={draft.genre ?? "Cinematic"} options={GENRE_OPTIONS} onChange={(v) => set({ genre: v })} />
                  </Row>
                  <Row label={t.aigc.mood}>
                    <MiniSelect value={draft.mood ?? "Uplifting"} options={MOOD_OPTIONS} onChange={(v) => set({ mood: v })} />
                  </Row>
                  <Row label={t.aigc.duration}>
                    <MiniSelect value={draft.duration ?? "30 sec"} options={MUSIC_DURATION_OPTIONS} onChange={(v) => set({ duration: v })} />
                  </Row>
                </>
              )}
            </div>
