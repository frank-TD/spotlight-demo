...
// Dependency-free WebGL "dark veil" — a slow, organic CPPN colour field forced
// through a black→gold duotone so it reads as flowing molten-gold light on ink,
// on-brand rather than the effect's native iridescence. Used as the stage of
...
const STATIC_TIME = 3.4; // fixed frame for reduced-motion
...
void main(){
    ...
    // black → gold duotone: shape luminance, then ramp ink → #d4af37.
    float l = dot(clamp(col.rgb,0.0,1.0), vec3(0.299,0.587,0.114));
    l = pow(l, uGamma);
    vec3 duo = mix(vec3(0.031,0.031,0.039), vec3(0.831,0.686,0.216), l);
...
export default function DarkVeil() {
  ...
  useEffect(() => {
    ...
    // Prefer WebGL2 (the ES 1.00 shader compiles in both); fall back to WebGL1.
    const gl =
    ...
    if (!gl) return; // no WebGL → section's own dark backdrop remains
    ...
    const resize = () => {
      const w = Math.max(1, Math.round(container.clientWidth * RES_SCALE));
      const h = Math.max(1, Math.round(container.clientHeight * RES_SCALE));
    ...
    if (reduced) {
      draw(STATIC_TIME); // one static frame, no animation
      return () => {
        ro.disconnect();
        gl.getExtension("WEBGL_lose_context")?.loseContext();
